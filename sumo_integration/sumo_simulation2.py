...
class SumoSimulation(object):
    ...
    def __init__(self, cfg_file, step_length, host=None, port=None, sumo_gui=False, client_order=1):
        ...
        if host is None or port is None:
            ...
            file_path = '/home/akua/Downloads/data.xlsx'
            ...
            #print(array)
            #while traci.simulation.getTime() > 0.1:
            #	for i in range(num_columns):
            #		for j in range(num_rows):      
            #			step = 0
            #			print("hi")
            #			#while 0.1 <= traci.simulation.getTime() <=15:
            #			if array[j][i] > 0:
            #				traci.vehicle.moveTo(data_array[0][i],"",array[j][i])
            #				step += 1
            #				print("hello")
            #				traci.simulationStep()
            				
            #simulation_steps = len(data_array)
            #traci.simulationStep()
            #for step in range(simulation_steps):
            #	positionX = data_array[step][0]
            #	positionY = data_array[step][1]
            #	angle = data_array[step][2]
            #	traci.vehicle.moveToXY("0", "", -1, positionX, positionY, angle)
            #	traci.simulationStep()
            #traci.close()
            ...
            #for i in range(25):
            #	traci.vehicle.moveToXY("0", "", 0, x, y)
            #	traci.vehicle.setSpeed("0", 20) 
            	#traci.simulationStep()

    
            
            #simulation_steps = len(data_array[0])
            #print(data_array)
            #simulation_steps = len(data_array[0]
            #num_vehicles = len(data_array)
            #for step in range(simulation_steps):
            #	for vehicle_id in range(1):
            #		if data_array[vehicle_id][step] > 0:
            #			position = data_array[vehicle_id][step]
            #			laneID= traci.vehicle.getLaneID(str(vehicle_id))
            #			traci.vehicle.moveTo(str(vehicle_id), laneID, position)
            #			print("hello")
            #			traci.simulationStep()
            ...
            #	for j in range
            #x, y = traci.vehicle.getPosition("0")
            #traci.vehicle.moveToXY("1", "", 0, 288, 204)
            #traci.vehicle.moveToXY("1", "", 0, 288, 204)
            #traci.simulationStep()
            #for i in range(1000):
            #	traci.vehicle.setSpeed("0", 20)
            #	traci.simulationStep()
            #traci.vehicle.moveToXY("2", "", 0, 295.2, 212.5)
            #traci.vehicle.setSpeed("2", 0) 
            #traci.vehicle.moveToXY("3", "", 0, 295.2, 205)
            #traci.vehicle.setSpeed("3", 0)
            #traci.vehicle.moveToXY("4", "", 0, 295.2, 199)   
            #traci.vehicle.setSpeed("4", 0)
            #traci.vehicle.moveToXY("5", "", 0, 295.2, 193)
            #traci.vehicle.setSpeed("5", 0)
            #traci.vehicle.moveToXY("6", "", 0, 295.2, 187.9)
            #traci.vehicle.setSpeed("6", 0)
            #traci.vehicle.moveToXY("7", "1", 0, 295.973, 199.656)
            #traci.vehicle.setSpeed("7", 0)
            
               
                                                                            
            #traci.vehicle.moveToXY("2", "", 0, 305.793, 199.975)
            #traci.vehicle.setSpeed("2", 0)            
                       
            	#traci.vehicle.setSpeed("19", 2)
            	#traci.vehicle.setSpeed("20", 14)
            	#traci.vehicle.setSpeed("11", 18)
            	#traci.vehicle.setSpeed("12", 2)
            	#traci.vehicle.setSpeed("13", 10)	
            	#traci.vehicle.setSpeed("14", 14)
            	#traci.vehicle.setSpeed("15", 12)
            	#traci.vehicle.setSpeed("16", 8)
            	#traci.vehicle.setSpeed("17", 6)
            	#traci.vehicle.setSpeed("18", 4)
            	#time.sleep(0.2)
            	#traci.simulationStep()	
            ...
            #while traci.simulation.getTime() < 75:
            	#angle = traci.vehicle.getAngle("31")
            	#x, y = traci.vehicle.getPosition("31")
            	#print("x=%s  y=%s angle=%s " % (x, y, angle))
            	#traci.simulationStep()	
            	#if step >= 10:
            		#angle2 = traci.vehicle.getAngle("0")
            		#print(angle2)
            	#x, y = traci.vehicle.getPosition("0")
            	#pos2 = (x-2, y)
            	#traci.vehicle.setSpeed("0", 2)
            	#traci.vehicle.setSpeed("11", 20)
            	#traci.vehicle.setSpeed("12", 25)
            	#traci.vehicle.moveToXY('0', "43.0.00", -1, pos2[0], pos2[1], 90, keepRoute=1)
            	#traci.simulationStep()
            	#time.sleep(0.2)	
            	#step += 1    
            #Code for deviating the pre-defined path    
            ...
            #while traci.simulation.getTime() < 120:
            	...
            	#traci.vehicle.setSpeed("1", 2)
            		...
            		#y = pos[1]
            		#pos2 = (pos[0]-2, pos[1])
            		#traci.vehicle.moveToXY('1', "45.0.00", -1, pos2[0], pos2[1],90,keepRoute=1)
            		#time.sleep(0.2)
            ...
            #while traci.simulation.getTime() < 120:
            		...
            		#traci.vehicle.setSpeed("2", 4)
            	#pos = traci.vehicle.getPosition("2")
            	#x = pos[0]
            	#y = pos[1]
            	#pos2 = (pos[0]+20, pos[1])
            	#traci.vehicle.movetoXY('2', "", -1, pos2[0], pos2[1], 0, keepRoute=1)
            	#time.sleep(0.2)
            	#time += 0.01
               	...
               	#Trying crash with another vehicle scenario
               	#pos = traci.vehicle.getPosition("1")
               	#traci.vehicle.moveToXY('4', "45.0.00", -1, pos[0], pos[1],0,keepRoute=1)
               	
               	#Trying the lane violation example
               	#pos = traci.vehicle.getPosition("1")
               	#x = pos[0]
               	#y = pos[1]
               	...
               	#traci.vehicle.moveToXY('1', "45.0.00", -1, pos2[0], pos2[1],0,keepRoute=1)
                       ...
                       #x = pos[0]
                       #y = pos[1]
                       #pos2 = (pos[0] + 0.5, pos[1])
                       #traci.vehicle.setSpeed("4", 20)
                       #traci.vehicle.moveToXY('1', "45.0.00", -1, pos2[0], pos2[1],
                       #0,keepRoute=1)
               #step += 1
            ...
            #traci.simulationStep()
            #while traci.simulation.getTime() < 75:
            	#pos = traci.vehicle.getPosition3D('1')
            	#pos2 = (pos[0] + 2, pos[1])
            	#traci.vehicle.moveToXY('1', '', 0, pos2[0], pos2[1])
            	#traci.simulationStep()
            	#pos3 = traci.vehicle.getPosition3D('1')
            #traci.close()
        else:
